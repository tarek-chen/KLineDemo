...
- (void)setData:(NSMutableArray *)data {
    ...
    if (_data != data) {
        _data = data;
        // 相对总数据源的起始下标
        NSInteger from = _data.count - _showCount;
        self.showCount = data.count >= _showCount ? _showCount : data.count;
        self.showIndex = from > 0 ? from : _data.count;
    }
    // 指标计算
...
- (void)getPathPoints {
    ...
    CGFloat candle_half = KlineStyle.shared.candle_w/2;
    ...
    [_needDraw enumerateObjectsUsingBlock:^(KLineModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        // 计算蜡烛坐标、高度
        [obj setCandleOriginWithHigh:self.maxPrice width:KlineStyle.shared.candle_w low:self.minPrice unitValue:unitDistance idx:idx maxY:maxY];
