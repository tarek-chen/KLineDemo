...
- (void)draw {
	...
	NSInteger unit = _models.count/4;
...
- (CATextLayer *)getPublicTextLayerWithY:(CGFloat)y {
    ...
    CATextLayer *publicLayer = [self getTextLayerWithString:nil textColor:KlineStyle.shared.textColor fontSize:8 backgroundColor:UIColor.clearColor frame:frame aligmentMode:kCAAlignmentLeft];
    return publicLayer;
}
...
- (void)colorTextLayer:(CATextLayer *)layer {
	...
	if (isMACD && subStr.count ==4) {
		...
		[attStr cat_ColorText:@[subStr[0]] color:KlineStyle.shared.color_1];
		[attStr cat_ColorText:@[subStr[1]] color:KlineStyle.shared.color_2];
		[attStr cat_ColorText:@[subStr[3]] color:KlineStyle.shared.color_3];
	}
	else {
		[attStr cat_ColorText:@[subStr.firstObject] color:KlineStyle.shared.color_1];
		if (subStr.count >1) {
			[attStr cat_ColorText:@[subStr[1]] color:KlineStyle.shared.color_2];
		}
		if (subStr.count >2) {
			[attStr cat_ColorText:@[subStr[2]] color:KlineStyle.shared.color_3];
		}
		
		if (isRSI && subStr.count >=6) {
			[attStr cat_ColorText:@[subStr[3]] color:KlineStyle.shared.color_1];
			[attStr cat_ColorText:@[subStr[4]] color:KlineStyle.shared.color_2];
			[attStr cat_ColorText:@[subStr[5]] color:KlineStyle.shared.color_3];
		}
	}
    // 关闭隐式动画
    [CATransaction begin];
