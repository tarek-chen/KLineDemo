...
- (void)draw {
    ...
    [_models enumerateObjectsUsingBlock:^(KLineModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
       ...
       if (!isLine) {
        ...
        // vol
        if (ENChartTypeVOL == KlineStyle.shared.bottomChartType) {
...
- (void)drawLineChart {
    ...
    // 顶部折线
    CAShapeLayer *line = [self drawLineWithPoints:_lines lineWidth:1 lineColor:KlineStyle.shared.color_Line];
    ...
    [points addObject:@(start)];
    CAShapeLayer *bg = [self drawRectWithPoints:points strokeColor:UIColor.clearColor fillColor:KlineStyle.shared.color_AlphaLine];
    [self addSublayer:bg];
}
...
- (void)drawCandleChart:(KLineModel *)obj {
    ...
    // 涨跌颜色 涨：close >open
    UIColor *candleColor = obj.close.floatValue > obj.open.floatValue ? KlineStyle.shared.color_up :KlineStyle.shared.color_dn;
    // 蜡烛
...
- (void)drawVolChart:(KLineModel *)obj {
    ...
    CGFloat candle_w = KlineStyle.shared.candle_w *KlineStyle.shared.scale;
    //        CGFloat halfCandleW = KlineStyle.shared.candle_w/2;
    ...
    UIColor *color = obj.close.floatValue > obj.open.floatValue ? KlineStyle.shared.color_up : KlineStyle.shared.color_dn;
