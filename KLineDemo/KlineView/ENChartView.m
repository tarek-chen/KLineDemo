...
- (void)longPress:(UILongPressGestureRecognizer *)ges {
    ...
    NSInteger idx = (point.x - kChartBorderWidth) / (KlineStyle.shared.candle_w + kCandleSpacing);
...
- (void)zoomAction:(UIPinchGestureRecognizer *)pinch {
    ...
    switch (pinch.state) {
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if (pinch.scale < KlineStyle.shared.minScale) {
                pinch.scale = KlineStyle.shared.minScale;
                break;
            }
            else if (pinch.scale > KlineStyle.shared.maxScale) {
               pinch.scale = KlineStyle.shared.maxScale;
                break;
            }
...
-(void)chartDidScroll:(UIPanGestureRecognizer *)panGest{
    ...
    if (UIGestureRecognizerStateChanged == panGest.state) {
        ...
        CGFloat candleW = (KlineStyle.shared.candle_w * KlineStyle.shared.scale);
...
-(void)updateView {
    ...
    if (_currentCount > _updateCount || _currentCount>60) {
        ...
        _total += point.x;
        NSInteger count = _total / (KlineStyle.shared.candle_w * KlineStyle.shared.scale);
        if (count != 0) {
